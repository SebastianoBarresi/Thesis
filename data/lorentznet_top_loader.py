...
class LorentzNetJetDataset(Dataset):
    """
    TensorDataset with support of transforms.
    """
    def __init__(self, data, num_pts=-1, shuffle=False, transform=None):
    ...
    def __getitem__(self, idx):
        ...
        ret = {key: val[idx] for key, val in self.data.items()}

        if self.transform:
            ret['Pmu'] = self.transform(ret['Pmu'])
            return ret
        else:    
            return ret
...
def initialize_top(datadir='data', num_pts=None, augmentation=None):
    """
    Initialize datasets.
    """
    ...
    if num_pts is None:
        splits = ['train', 'valid', 'test'] # Our data categories -- training set, testing set and validation set
    ...
    torch_datasets = {split: ConcatDataset([LorentzNetJetDataset(data, num_pts=num_pts_per_file[split][idx], transform=augmentation) 
                                            for idx, data in enumerate(datasets[split])]) for split in splits}
...
def collate_fn_top(data, scale=1., nobj=None, edge_features=[], add_beams=False, beam_mass=1):
    """
